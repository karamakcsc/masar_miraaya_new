frappe.pages['order-delivery'].on_page_load = function(wrapper) {
	...
	$barcode_section.find('.barcode-input').on('keypress', function(e) {
		...
		frappe.call({
			method: 'masar_miraaya_new.masar_miraaya_new.page.order_delivery.order_delivery.get_so_dispatch_details',
			args: {
	...
	$orders_container.on('click', '.assign-driver-btn', function() {
		...
		frappe.confirm(
			...
			function() {
				frappe.call({
					method: 'masar_miraaya_new.masar_miraaya_new.page.order_delivery.order_delivery.assign_driver_and_dispatch',
					args: {
