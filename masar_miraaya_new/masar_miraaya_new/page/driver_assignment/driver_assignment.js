frappe.pages['driver-assignment'].on_page_load = function(wrapper) {
	...
	$barcode_section.find('.barcode-input').on('keypress', function(e) {
		...
		frappe.call({
			method: 'masar_miraaya_new.masar_miraaya_new.page.driver_assignment.driver_assignment.get_so_dispatch_details',
			args: {
			...
			callback: function(r) {
				if (!r.message || r.message.length === 0) {
					frappe.show_alert({
						message: __('Only Fullfilled Sales Orders are allowed'),
						indicator: 'red'
					});
					return;
				}
	...
	$orders_container.on('click', '.assign-driver-btn', function() {
		...
		frappe.confirm(
			__('Assign driver and dispatch selected Sales Orders?'),
			function() {
				frappe.call({
					method: 'masar_miraaya_new.masar_miraaya_new.page.driver_assignment.driver_assignment.assign_driver_and_dispatch',
					args: {
					...
					callback: function(r) {
						if (r.message && r.message.status === 'success') {
							frappe.msgprint({
								title: __('Success'),
								message: __(`${r.message.count} Sales Orders dispatched successfully`),
								indicator: 'green'
							});
							scanned_orders = {};
							render_scanned_list();
