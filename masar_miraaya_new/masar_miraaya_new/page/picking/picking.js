frappe.pages['picking'].on_page_load = function(wrapper) {
	...
	// main container
	let $container = $('<div class="picking-container"></div>').appendTo(page.main);
	...
	function render_orders(order) {
		...
		frappe.call({
			method: 'masar_miraaya_new.masar_miraaya_new.page.picking.api.get_confirmed_sales_orders',
			args: {
				so: order
			},
			callback: function(r) {
	...
	function open_pick_list(sales_order) {
		frappe.call({
			method: 'masar_miraaya_new.masar_miraaya_new.page.picking.api.get_picklist_from_so',
			args: {
				so: sales_order
			},
			callback: function(r) {
